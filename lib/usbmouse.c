...
{
	printf("USB_MOUSE: In mouse device configure. \n");
	...
	printf("HID descriptor length : %d", pHIDDesc->wReportDescriptorLength);
	...
	if (DWHCIDeviceControlMessage(USBFunctionGetHost(&pThis->m_USBFunction),
								  ...
								  pThis->m_pHIDReportDescriptor, pThis->m_usReportDescriptorLength) != pThis->m_usReportDescriptorLength)
	{
...
boolean USBMouseDeviceStartRequest(TUSBMouseDevice *pThis)
{
	...
	// printf("Mouse pThis->m_pReportEndpoint:%d \n", pThis->m_pReportEndpoint);
...
void USBMouseDeviceCompletionRoutine(TUSBRequest *pURB, void *pParam, void *pContext)
{
	...
	if (USBRequestGetStatus(pURB) != 0 && USBRequestGetResultLength(pURB) == MOUSE_BOOT_REPORT_SIZE && pThis->m_pStatusHandler != 0)
	{
		assert(pThis->m_pReportBuffer != 0);
