...
boolean USBDeviceInitialize(TUSBDevice *pThis)
{
	...
	if (DWHCIDeviceGetDescriptor(pThis->m_pHost, pThis->m_pEndpoint0,
								 ...
								 pThis->m_pDeviceDesc, USB_DEFAULT_MAX_PACKET_SIZE, REQUEST_IN) != USB_DEFAULT_MAX_PACKET_SIZE)
	{
	...
	if (pThis->m_pDeviceDesc->bLength != sizeof *pThis->m_pDeviceDesc || pThis->m_pDeviceDesc->bDescriptorType != DESCRIPTOR_DEVICE)
	{
	...
	printf("Dev desc length: %d \n", pThis->m_pDeviceDesc->bLength);
	USBEndpointSetMaxPacketSize(pThis->m_pEndpoint0, pThis->m_pDeviceDesc->bMaxPacketSize0);

	if (DWHCIDeviceGetDescriptor(pThis->m_pHost, pThis->m_pEndpoint0,
								 DESCRIPTOR_DEVICE, DESCRIPTOR_INDEX_DEFAULT,
								 pThis->m_pDeviceDesc, sizeof *pThis->m_pDeviceDesc, REQUEST_IN) != (int)sizeof *pThis->m_pDeviceDesc)
	{
	...
	if (pThis->m_pDeviceDesc->iManufacturer != 0 || pThis->m_pDeviceDesc->iProduct != 0)
	{
	...
	if (DWHCIDeviceGetDescriptor(pThis->m_pHost, pThis->m_pEndpoint0,
								 ...
								 pThis->m_pConfigDesc, sizeof *pThis->m_pConfigDesc, REQUEST_IN) != (int)sizeof *pThis->m_pConfigDesc)
	{
	...
	if (pThis->m_pConfigDesc->bLength != sizeof *pThis->m_pConfigDesc || pThis->m_pConfigDesc->bDescriptorType != DESCRIPTOR_CONFIGURATION || pThis->m_pConfigDesc->wTotalLength > MAX_CONFIG_DESC_SIZE)
	{
	...
	if (DWHCIDeviceGetDescriptor(pThis->m_pHost, pThis->m_pEndpoint0,
								 DESCRIPTOR_CONFIGURATION, DESCRIPTOR_INDEX_DEFAULT,
								 pThis->m_pConfigDesc, nTotalLength, REQUEST_IN) != (int)nTotalLength)
	{
	...
	assert(pNames != 0);
	printf("nTotalLength: %d device:%s \n", nTotalLength, StringGet(pNames));
...
TString *USBDeviceGetName(TUSBDevice *pThis, TDeviceNameSelector Selector)
{
	...
	case DeviceNameDevice:
		assert(pThis->m_pDeviceDesc != 0);
		if (pThis->m_pDeviceDesc->bDeviceClass == 0 || pThis->m_pDeviceDesc->bDeviceClass == 0xFF)
		{
			goto unknown;
		}
